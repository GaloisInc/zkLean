...
| `(tactic| bvify $[[$simpArgs,*]]? $[at $location]?) =>
  ...
  `(tactic|
    simp -decide only [ BitVec.ofNat_if_then_else, BitVec.ofNat_if_then_prop_else , BitVec.ofNat_add, BitVec.ofNat_mul, BitVec.ofNat_sub, BitVec.ofNat_toNat, push_cast, $args,*] $[at $location]? )
...
def mkZifyContext (simpArgs : Option (Syntax.TSepArray `Lean.Parser.Tactic.simpStar ",")) :
  ...
  let args := simpArgs.map (·.getElems) |>.getD #[]
  mkSimpContext
    (← `(tactic| simp -decide only  [ BitVec.ofNat_if_then_else, BitVec.ofNat_if_then_prop_else , BitVec.ofNat_add, BitVec.ofNat_mul, BitVec.ofNat_sub, BitVec.ofNat_toNat, push_cast, $args,*] )) false
