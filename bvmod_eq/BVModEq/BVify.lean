...
lemma BitVec.ofNat_sub  {bw x : ℕ}  (h : y ≥ x) (h1: x < 2 ^ bw) (h2: y < 2 ^ bw)  :
...
| `(tactic| bvify $[[$simpArgs,*]]? $[at $location]?) =>
  ...
  `(tactic|
    simp -decide only [ult_bv, or_bv, BitVec.ofNat_if_then_else, BitVec.ofNat_if_then_prop_else , BitVec.ofNat_add, BitVec.ofNat_mul, BitVec.ofNat_sub, BitVec.ofNat_toNat, push_cast, $args,*] $[at $location]? )
...
def mkZifyContext (simpArgs : Option (Syntax.TSepArray `Lean.Parser.Tactic.simpStar ",")) :
  ...
  let args := simpArgs.map (·.getElems) |>.getD #[]
  mkSimpContext
    (← `(tactic| simp -decide only  [ult_bv, or_bv, BitVec.ofNat_if_then_else, BitVec.ofNat_if_then_prop_else , BitVec.ofNat_add, BitVec.ofNat_mul, BitVec.ofNat_sub, BitVec.ofNat_toNat, push_cast, $args,*] )) false
