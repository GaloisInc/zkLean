...
| `(tactic| valify $[[$simpArgs,*]]? $[at $location]?) =>
  ...
  `(tactic|
    simp [ ZMod.if_then_else_val, ZMod.if_then_else_prop_val, ZMod.val_sub, ZMod.val_add, ZMod.val_mul, ZMod.val_one, ZMod.val_zero, ZMod.val_ofNat, push_cast, $[$args],*] $[at $location]? )
...
def mkZifyContext (simpArgs : Option (Syntax.TSepArray `Lean.Parser.Tactic.simpStar ",")) :
  ...
  let args := simpArgs.map (·.getElems) |>.getD #[]
  mkSimpContext
    (← `(tactic| simp  [ ZMod.if_then_else_val,ZMod.if_then_else_prop_val, ZMod.val_sub, ZMod.val_add, ZMod.val_mul, ZMod.val_one, ZMod.val_zero,  ZMod.val_ofNat, push_cast, $[$args],*] )) false
