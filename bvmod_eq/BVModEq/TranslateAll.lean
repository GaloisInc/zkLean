...
lemma ZMod.val_le_BV {n : ℕ} [k: NeZero n] (a : ZMod n) (w : ℕ ) (h: n< 2^w) : BitVec.ofNat w (a.val) ≤ BitVec.ofNat w (n) := by
   unfold BitVec.ofNat
   simp
   rw [Nat.mod_eq_of_lt]
   rw [Nat.mod_eq_of_lt]
   apply ZMod.toNatLT
   apply Nat.pos_of_ne_zero (k.ne n)
   apply h
   apply (lt_trans (ZMod.val_lt a) h)
