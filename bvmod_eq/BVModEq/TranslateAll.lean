...
elab_rules : tactic
  ...
  withMainContext do
    ...
    | none => logInfo m!"{goalTy} not applicable"
    | some (exp, n) =>
        if k < n then
        -- A : Prop := exp < k
...
| `(tactic| translate_goal [$ids,*] $[$b:term]? ) => withMainContext do
 ...
 -- if i > 0 then
  ...
  evalTactic (← `(tactic| try rw [<- sub_eq_add_neg]))
  evalTactic (← `(tactic| try rw [neg_add_to_sub]))
  --evalTactic (← `(tactic| try simp [-neg_mul, -one_mul, -mul_one] ))
  if k > 0 then
    subLoop := true
    while (subLoop) do
      try
  ...
  try
      ...
      --logInfo m!"ERROR BAD: {e.toMessageData}"
      pure ()
  --     --rmFailed := true
        ...
        match getVarEq hType with
          | some rhsVarId => do
              ...
              try
                ...
                evalTactic (← `(tactic| try simp at $(mkIdent newName.getId):ident))
...
-- variable (a : BitVec 15)
-- variable (x_bit14 : FF0)
-- variable (x_bit13 : FF0)
-- variable (x_bit12 : FF0)
-- variable (x_bit11 : FF0)
-- variable (x_bit10 : FF0)
-- variable (x_bit9 : FF0)
-- variable (x_bit8 : FF0)
-- variable (x_bit7 : FF0)
-- variable (x_bit6 : FF0)
...
--   simp at h13_new
--   simp at h14_new
--   --translat_goal [h0_new, h1_ne1, h2_new, h3_new, h4_new, h5_new, h6_new, h7_new, h9_new, h10_new, h11_new, h12_new, h13_new, h14_new]
--   --focus try_apply_lemma_hyps  [h0_new, h1_new, h2_new, h3_new, h4_new, h5_new, h6_new, h7_new, h9_new, h10_new, h11_new, h12_new, h13_new, h14_new]


--  valify [] at h16
--  rw [ZMod.val_sub_strict] at h16
--  rw [Nat.mod_eq_of_lt]
-- abbrev ffff0 := 52435875175126190479447740508185965837690552500527637822603658699938581184513
-- instance : Fact (Nat.Prime ffff0) := by sorry
-- instance : Fact (NeZero ffff0) := by sorry
...
--   simp
  ...
  -- rw [Nat.mod_eq_of_lt]
  -- rw [BVModEq.BitVec_ofNat_eq_iff 13]
  -- rw [Nat.mod_eq_of_lt]
  -- -- sorry
  -- -- swap
  -- -- sorry
  ...
  -- example: (((((((((((((if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[0] = true then 1 else 0) +
  --                             if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[1] = true then 2 else 0) +
  --                           if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[2] = true then 4 else 0) +
  --                         if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[3] = true then 8 else 0) +
  --                       if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[4] = true then 16 else 0) +
  --                     if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[5] = true then 32 else 0) +
  --                   if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[6] = true then 64 else 0) +
  --                 if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[7] = true then 128 else 0) +
  --               if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[8] = true then 256 else 0) +
  --             if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[9] = true then 512 else 0) +
  --           if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[10] = true then 1024 else 0) +
  --         if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[11] = true then 2048 else 0) +
  --       if (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[12] = true then 4096 else 0) %
  --     52435875175126190479447740508185965837690552500527637822603658699938581184513 =
  --   (a.toNat + 4095) % 52435875175126190479447740508185965837690552500527637822603658699938581184513 -
  --     b.toNat % 52435875175126190479447740508185965837690552500527637822603658699938581184513 ∧
  -- (BitVec.setWidth 13 a + 4095#13 - BitVec.setWidth 13 b)[12] = b.ult a := by
  -- rw [Nat.mod_eq_of_lt]
  -- swap
  -- sorry
  -- rw [BVModEq.BitVec_ofNat_eq_iff 13]
  ...
  -- -- all_goals try rw [<- sub_eq_add_neg]
  -- -- all_goals try rw [neg_param]
  -- -- all_goals try simp [sub_add_right_recursive]
  -- -- all_goals try simp [Nat.mul_assoc]
