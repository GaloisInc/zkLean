...
elab_rules : tactic
  ...
  withMainContext do
    ...
    | none => pure ()--logInfo m!"{goalTy} not applicable"
    | some (exp, n) =>
        if k < n then
        -- A : Prop := exp < k
